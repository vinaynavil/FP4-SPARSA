...
def metric_tile(parent, title, var, color, bg_color=C_TILE, label_color=T_MUTED, size=19):
    ...
    tk.Label(f, textvariable=var, bg=bg_color, fg=color, font=font_spec(size, "bold", mono=True)).pack(anchor="w",
                                                                                                       pady=(2, 0))
    return f
...
class FP4AcceleratorDashboard:
    ...
    def _build_header(self):
        ...
        tk.Label(right, textvariable=self.status_var, bg=C_CARD, fg=T_SUB, font=font_spec(9, mono=True)).pack(
            side="left")
        ...
        badges = [
            ...
            ("EFFICIENCY: 290.9 GOPS/W", C_EMERALD_BG, C_EMERALD_L),
    ...
    def _build_left_panel(self, parent):
        ...
        tk.Label(info, textvariable=self.true_lbl, bg=C_CARD, fg=T_MAIN, font=font_spec(16, "bold")).pack(anchor="w",
                                                                                                          pady=(2, 12))

        # FP32 Prediction Tile
        f32 = tk.Frame(info, bg=C_EMERALD_BG, padx=12, pady=10)
        f32.pack(fill="x", pady=(0, 8))
        tk.Label(f32, text="FP32 BASELINE MODEL", bg=C_EMERALD_BG, fg=C_EMERALD_L, font=font_spec(8, "bold")).pack(
            anchor="w")
        self.pred_fp32 = tk.StringVar(value="—")
        tk.Label(f32, textvariable=self.pred_fp32, bg=C_EMERALD_BG, fg=C_EMERALD, font=font_spec(14, "bold")).pack(
            anchor="w")
        self.conf_fp32 = tk.StringVar(value="—")
        tk.Label(f32, textvariable=self.conf_fp32, bg=C_EMERALD_BG, fg=T_SUB, font=font_spec(8, mono=True)).pack(
            anchor="w")

        # FP4 Quantized Tile
        f4 = tk.Frame(info, bg=C_CYAN_BG, padx=12, pady=10)
        f4.pack(fill="x")
        tk.Label(f4, text="FP4 SPARSA ACCELERATOR", bg=C_CYAN_BG, fg=C_CYAN_L, font=font_spec(8, "bold")).pack(
            anchor="w")
        ...
        self.rand_btn = tk.Button(btn_box, text="⟳  Random Vector Sample", command=self._random_image,
                                  ...
                                  activeforeground=C_CYAN_L, relief="flat", pady=8, state="disabled", cursor="hand2",
                                  bd=0)
    ...
    def _build_right_panel(self, parent):
        ...
        for name, attr, color in [("FP32 Baseline Accuracy", "bar_fp32", C_EMERALD),
                                  ("FP4 Quantized Accuracy", "bar_fp4", C_CYAN)]:
            r = tk.Frame(card1, bg=C_CARD)
            r.pack(fill="x", padx=18, pady=6)
            tk.Label(r, text=name, bg=C_CARD, fg=T_MAIN, font=font_spec(9, "bold"), width=22, anchor="w").pack(
                side="left")
        ...
        card_header(card2, "2. Sparsity Profiling")

        grid2 = tk.Frame(card2, bg=C_CARD)
        grid2.pack(fill="x", padx=18, pady=(0, 10))
        for i in range(3): grid2.columnconfigure(i, weight=1)
        ...
        for name, attr, color in [("Weight Sparsity", "sp_wgt", C_EMERALD), ("Activation Sparsity", "sp_act", C_CYAN),
                                  ("MACs Skipped", "sp_mac", C_GOLD)]:
        ...
        hw_metrics = [
            ...
            ("Timing Slack (WNS)", "+0.205 ns", C_EMERALD, C_EMERALD_BG, C_EMERALD_L),
            ("Peak Throughput", "44.8 GOPS", C_CYAN, C_CYAN_BG, C_CYAN_L),
            ("Energy Efficiency", "290.9 GOPS/W", C_EMERALD, C_EMERALD_BG, C_EMERALD_L),
            ("DSP48E1 Slices", "80 / 600 (13.33%)", T_SUB, C_TILE, T_MUTED),
            ("Slice LUT Footprint", "1,445 (1.43%)", T_SUB, C_TILE, T_MUTED),
            ("Flip-Flop Storage", "3,380 (1.64%)", T_SUB, C_TILE, T_MUTED),
            ("Block RAM (BRAM36)", "2 (0.62%)", T_SUB, C_TILE, T_MUTED),
            ("Total On-Chip Power", "0.154 W (154 mW)", C_GOLD, C_GOLD_BG, C_GOLD_L),
        ...
        tk.Label(footer,
                 text=("• GOPS/W evaluated on Total Power (0.154 W)  "
